...
import { ArrowRightIcon, HistoryIcon, Loader2Icon, TimerIcon, Wand2Icon, XIcon } from "lucide-react";
...
const AIComposer = () => {
  ...
  const handleSchedule = async () => {
    ...
    setTimeout(()=> {
      setScheduling(false)
      // Snapshot the date/time before the form state gets reset
    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-40" placeholder="Type your ideas here... (e.g. A post about the weekend spent on the mountains)" value={prompt} onChange={(e)=> setPrompt(e.target.value)}/>
            <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">

              <button onClick={()=> setGenerateImage(!generateImage)} className="flex items-center gap-3 border border-slate-300 hover:bg-gray-50 py-2 px-3 rounded-xl">
                <span className="text-slate-700">AI Image</span>
              ...
              <button onClick={handleGenerate} disabled={loading} className="bg-slate-900 hover:bg-slate-800 text-white flex items-center gap-2 px-4 py-2 rounded-xl">
                {loading ? (
                  <>
        ...
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30">
              ...
              <button onClick={closeScheduler} className="p-2 rounded-full hover:bg-slate-100 text-slate-400 transition-colors">
                <XIcon className="size-5"/>
              </button>
            </div>

            <div className="flex-1 overflow-y-auto overflow-x-visible p-8 space-y-4">
              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                <p className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap">{activeScheduler.prompt}</p>
              </div>

              <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4">
                <p className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap">{activeScheduler.content}</p>
            ...
            <div className="p-8 bg-slate-50/50 border-t border-slate-50 space-y-6 overflow-visible">
              ...
              <button
                ...
                disabled={scheduling || selectedPlatforms.length === 0 || !scheduledDate || !scheduledTime}
                className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-red-500 hover:text-white transition disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:bg-slate-200 disabled:hover:text-slate-700"
              >
