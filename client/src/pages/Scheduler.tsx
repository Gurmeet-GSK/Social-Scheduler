...
import { ArrowRightIcon, Calendar1Icon, CalendarDaysIcon, Clock10Icon, SendIcon, XIcon } from "lucide-react";
          ...
          <div className="flex items-center gap-2 mb-6">
            ...
            <form className="space-y-5" action="" onSubmit={handleSchedule}>
              ...
              <div>
                ...
                <textarea className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content} onChange={(e)=>setContent(e.target.value)} name="" id="" required rows={5} placeholder="What do you want to share today"/>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs text-slate-500 uppercase mb-2">Date</label>
                  <div className="relative">
                    <Clock10Icon className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"/>
              ...
              {/*Submit*/}
              <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-3.5 bg-red-600 transition-all text-white rounded-lg ">
                {loading ? (
                  <>
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
            {scheduled.length===0 ? (
              <div className="py-10 text-center text-slate-400 text-sm">No psots scheduled yet</div>
            ) : (
              scheduled.map((post)=>(
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
            {published.length===0 ? (
              <div className="py-10 text-center text-slate-400 text-sm">No published psot yet</div>
            ) : (
              published.map((post)=>(
                <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                  <div className="flex items-center justify-between mb-2">
