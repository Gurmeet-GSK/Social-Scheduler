        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <form className="space-y-5" onSubmit={handleSchedule}>
            ...
            <div>
              ...
              <textarea className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content} onChange={(e) => setContent(e.target.value)} required rows={5} placeholder="What do you want to share today" />
            ...
            {/*Date & Time*/}
            <div>
              <label className="block text-xs text-slate-500 uppercase mb-2">Date & Time</label>
              <DateTimePicker
            ...
            <button
              type="submit"
              disabled={loading || selectedPlatforms.length === 0 || !scheduledDate || !scheduledTime}
              className="w-full flex items-center justify-center gap-2 py-3.5 bg-red-600 transition-all text-white rounded-lg disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {loading ? (
                <>
