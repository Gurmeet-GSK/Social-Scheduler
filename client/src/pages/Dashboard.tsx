import { ActivityIcon, CheckCircleIcon, Clock10Icon, SendIcon, Share2Icon, TrendingUpIcon } from "lucide-react";
...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                const [postsRes, accountRes, actitvityRes] = await Promise.all([api.get("/api/posts"), api.get("/api/accounts"), api.get("/api/activity")])
                ...
                setActivities(actitvityRes.data)
            } catch (error: any) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols lg:grid-cols-4 gap-5">
                {statCards.map((cards) => (
                    <div key={cards.label} className="bg-white hover:bg-red-50 relative border border-slate-200 rounded-2xl p-5 hover:border-red-200 transition-all">
                        <div className="flex items-center justify-between mb-4">
                            <div className="text-3xl font-medium text-slate-800 tabular-nums">{cards.value}</div>
                            <div className="text-xs absolute right-4 top-4 text-red-500 flex items-center gap-1">
                                <TrendingUpIcon className="size-3" />
                                {cards.trend}
                            </div>
                        </div>
                        <p className="test-sm text-slate-500 mt-1">{cards.label}</p>
                    </div>
                ))}
            </div>
