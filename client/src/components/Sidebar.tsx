import { CalendarSearchIcon, LayoutDashboardIcon, LogOutIcon, User2Icon, Wand2Icon } from "lucide-react";
import { NavLink, useLocation } from "react-router-dom";

const Sidebar = ({isOpen, setIsOpen}: {isOpen: boolean; setIsOpen: (val: boolean) => void}) => {
    const   {logout, user} = {
        logout: ()=> {window.location.href = "/";},
        user: {name: "Guri", email: "guri@gmail.com"}
    }
   
    const location = useLocation();
    const navItems = [
        {name: "Dashboard", icon : LayoutDashboardIcon, path : "/dashboard"},
        {name: "Accounts", icon : User2Icon, path : "/accounts"},
        {name: "Scheduler", icon : CalendarSearchIcon, path : "/scheduler"},
  ...
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
     ...
     <div className="p-4 border-t border-slate-100">
        ...
        <button onClick={logout} className="mt-1 flex items-center gap-2 px-3 py-2 w-full rounded text-sm text-slate-500 hover:bg-red-100 hover:text-red-500 transition-all duration-150">
